...
static int do_TPM2_Device(void* userCtx, int argc, char *argv[])
{
    ...
    if (argc < 1 || argc > 2) {
        printf("Error: Expected 1-2 command arguments but got %d\n", argc);
        return CMD_RET_USAGE;
    }
    printf("TPM2 Device Info\n");

    if (argc == 2) {
		num = dectoul(argv[1], NULL);
		rc = tpm_set_device(num);
		if (rc != TPM_RC_SUCCESS)
			printf("tpm_set_device: could not set TPM %lu (rc = %d)\n",
                num, rc);
	} else {
		rc = tpm_show_device();
	}
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_Device complete rc = %d\n", rc);
    #endif
    return rc;
}
...
static int do_TPM2_Info(void* userCtx, int argc, char *argv[])
{
    ...
    /* Need 1 arg: command */
    if (argc != 1) {
        printf("Error: Expected 1 command argument but got %d\n", argc);
        return CMD_RET_USAGE;
    }
    printf("TPM2 Device Info\n");
...
static int do_TPM2_AutoStart(void* userCtx, int argc, char *argv[])
{
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_AutoStart complete rc = %d\n", rc);
    #endif
    return rc;
}
...
static int do_TPM2_Init(void* userCtx, int argc, char *argv[])
{
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_Init complete rc = %d\n", rc);
    #endif
    return rc;
}
...
static int do_TPM2_Wrapper_CapsArgs(void* userCtx, int argc, char *argv[])
{
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_Wrapper_CapsArgs complete rc = %d\n", rc);
    #endif
    return rc;
}
...
static int do_TPM2_Firmware_Update(void* userCtx, int argc, char *argv[])
{
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_Firmware_Update complete\n");
    #endif
    return rc;
}
...
static int do_TPM2_Firmware_Cancel(void* userCtx, int argc, char *argv[])
{
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_Firmware_Cancel complete\n");
    #endif
    return rc;
}
#endif /* WOLFTPM_SLB9672 || WOLFTPM_SLB9673 */
...
static int do_TPM2_Startup(void* userCtx, int argc, char *const argv[])
{
    ...
    if (rc >= TPM_RC_SUCCESS) rc = TPM_RC_SUCCESS;
    printf("do_TPM2_Startup complete (%s) rc = %d\n",
...
static int do_TPM2_SelfTest(void* userCtx, int argc, char *const argv[])
{
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_SelfTest complete (%s) rc = %d\n",
            fullTest ? "full" : "continue", rc);
    #endif
    return rc;
}
...
static int do_TPM2_Clear(void* userCtx, int argc, char *const argv[])
{
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_Clear complete (%s) rc = %d\n",
...
static int do_TPM2_PCR_Extend(void* userCtx, int argc, char *const argv[])
{
    ...
    if (argc < 3 || argc > 4) {
        printf("Error: Expected 3-4 arguments but got %d\n", argc);
        return CMD_RET_USAGE;
    }
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_PCR_Extend complete rc = %d\n", rc);
    #endif
    return rc;
}
...
static int do_TPM2_PCR_Read(void* userCtx, int argc, char *const argv[])
{
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_PCR_Read complete rc = %d\n", rc);
    #endif
    return rc;
}
...
static int do_TPM2_PCR_Allocate(void* userCtx, int argc, char *const argv[])
{
    ...
    wolfTPM2_Cleanup(&dev);
        ...
        printf("do_TPM2_PCR_Allocate complete: %s (%s), rc = %d\n",
            TPM2_GetAlgName(in.pcrAllocation.pcrSelections[0].hash),
            argv[2], rc);
    #endif
    return rc;
    ...
    int isPolicy)
{
    ...
    printf("TPM2 PCR Set Auth for %s\n", argv[1]);
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_PCR_SetAuth%s complete: %s, rc = %d\n",
            isPolicy ? "Policy" : "Value",
...
static int do_TPM2_Change_Auth(void* userCtx, int argc, char *const argv[])
{
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_Change_Auth complete rc = %d\n", rc);
    #endif
    return rc;
}
...
static int do_TPM2_PCR_Print(void* userCtx, int argc, char *argv[])
{
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_PCR_Print complete rc = %d\n", rc);
    #endif
    return rc;
}
...
static int do_TPM2_Dam_Reset(void* userCtx, int argc, char *const argv[])
{
    ...
    if (argc > 2) {
        printf("Error: Expected 1-2 arguments but got %d\n", argc);
        return CMD_RET_USAGE;
    }
    printf("TPM2 DAM Reset\n");
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_Dam_Reset complete rc = %d\n", rc);
    #endif
    return rc;
}
...
static int do_TPM2_Dam_Parameters(void* userCtx, int argc, char *const argv[])
{
    ...
    if (argc < 4 || argc > 5) {
        printf("Error: Expected 4-5 arguments but got %d\n", argc);
        return CMD_RET_USAGE;
    }
    printf("TPM2 DAM Parameters\n");
    ...
    #ifdef DEBUG_WOLFTPM
        printf("do_TPM2_Dam_Parameters complete rc = %d\n", rc);
    #endif
    return rc;
}
